...
if __name__ == "__main__":
    ...
    bridges_dir = os.path.join(ROOT_DIR, "data/inspection_reports/data")
    bridge_dirs = os.listdir(bridges_dir)
    for bridge_dir in bridge_dirs:
        bridge_dir = os.path.join(bridges_dir, bridge_dir)
        if not os.path.isdir(bridge_dir):
            continue

        for i, file_name in enumerate(os.listdir(bridge_dir)):
            # ignore non-pdf files
            if not file_name.endswith(".pdf"):
                continue

            logger.log(
                f"({i+1} of {len(os.listdir(bridge_dir))}) Preparing inspection report: {file_name}"
            )
            ...
            file_path = os.path.join(bridge_dir, file_name)
